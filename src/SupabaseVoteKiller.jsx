// App.jsx – QR moved to top, top‑3 below; new video ID hooHIkOQXdg
// -------------------------------------------------------------------------------------------------
...
function ResultsPage() {
  ...
  const VIDEO_ID = "hooHIkOQXdg"; // new video
  const poll = async () => {
  ...
  const { top3, total } = useMemo(()=>{
    ...
    return { total, top3: arr.sort((a,b)=>b.count-a.count).slice(0,3).map(o=>({...o,pct: total?((o.count/total)*100).toFixed(1):"0"})) };
  }, [results]);
  ...
  return (
    <div className="h-screen w-screen flex overflow-hidden">
      ...
      <div className="flex-none w-[10vw] min-w-[160px] bg-white p-1 flex flex-col items-center gap-2 overflow-hidden">
        {/* QR + CTA at top */}
        <p className="text-[11px] font-bold text-center leading-tight">Who do <em>you</em> think is the real killer?<br/>Vote at:</p>
        <div className="w-[90%]"><QRCode value="https://leo-cazenille.github.io/GUESS-THE-KILLER/" size={128} style={{ width:"100%", height:"auto" }} /></div>

        {/* Explanation */}
        <p className="text-[11px] font-semibold text-center px-1 leading-tight mt-2">Characters currently receiving the most votes:</p>

        {/* Top‑3 list */}
        <div className="flex-1 w-full flex flex-col items-center gap-2 overflow-y-auto mt-1">
          {top3.map((itm,idx)=>(
            <div key={idx} className="w-full flex flex-col items-center">
              <img src={itm.src} alt={itm.name} className="w-[85%] aspect-[2/3] object-cover rounded-md border" />
              <p className="text-[10px] text-center font-semibold leading-tight mt-1 px-1">{itm.name}</p>
              <p className="text-[10px] font-medium">{itm.pct}%</p>
            </div>
          ))}
        </div>
