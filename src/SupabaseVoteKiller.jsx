// App.jsx – responsive right-column width + smaller fonts in VoteGrid
// -------------------------------------------------------------------------------------------------
...
// ---------- VoteGrid with original font sizes ------------------------------
function VoteGrid() {
...
function ResultsPage() {
  ...
  const { display, total } = useMemo(()=>{ const arr=IMAGES.map(i=>({ ...i, count: results.find(r=>r.image_id===i.id)?.count||0 })); const total=arr.reduce((a,b)=>a+b.count,0); if(!total) return{total,display:[]}; const nonZero=arr.filter(e=>e.count>0).sort((a,b)=>b.count-a.count).slice(0,3); return{ total, display: nonZero.map(o=>({...o,pct:((o.count/total)*100).toFixed(1)})) }; },[results]);
  return (
    <div className="h-screen w-screen flex overflow-hidden">
      ...
      <div className="flex-none w-full sm:w-[30vw] lg:w-[20vw] xl:w-[15vw] 2xl:w-[10vw] min-w-[200px] bg-white p-2 flex flex-col items-center gap-3 overflow-hidden">
        {/* QR heading */}
        <p className="text-xl font-bold text-center leading-tight text-black">Who is the real killer? Vote at:</p>
        <div className="w-[90%]"><QRCode value="https://leo-cazenille.github.io/GUESS-THE-KILLER/" size={140} style={{ width:"100%", height:"auto" }} /></div>

        {/* Top‑3 heading */}
        <p className="text-lg font-semibold text-center text-black">Top 3 most voted:</p>

        {display.length===0 ? (
          <p className="text-xl italic text-black mt-4">No votes yet</p>
        ) : (
            ...
            {display.map((itm,idx)=>(
              <div key={idx} className="w-full flex flex-col items-center">
                <div className="relative w-[60%] aspect-[2/3]">
                  ...
                  <span className="absolute bottom-0 left-0 w-full bg-black/70 text-white text-center text-sm font-bold py-0.5 uppercase tracking-wider">{itm.name}</span>
                </div>
                <p className="text-xl font-bold text-black">{itm.pct}%</p>
              </div>
            ))}
          </div>
        )}
        <p className="text-lg text-center text-black mb-1">{total} total votes</p>
      </div>
    </div>
  );
}
