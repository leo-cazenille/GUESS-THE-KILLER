// App.jsx – responsive font & image sizes in right column
// -------------------------------------------------------------------------------------------------
...
function ResultsPage(){
  ...
  const {display,total}=useMemo(()=>{const arr=IMAGES.map(i=>({...i,count:results.find(r=>r.image_id===i.id)?.count||0}));const total=arr.reduce((a,b)=>a+b.count,0);if(total===0)return{total,display:[]};const nonZero=arr.filter(e=>e.count>0).sort((a,b)=>b.count-a.count);const top=nonZero.length>3?nonZero.slice(0,3):nonZero;return{total,display:top.map(o=>({...o,pct:((o.count/total)*100).toFixed(1)}))};},[results]);
  return(
    <div className="h-screen w-screen flex overflow-hidden">
      ...
      <div className="flex-none w-[10vw] min-w-[260px] bg-white p-2 flex flex-col items-center gap-4 overflow-hidden">
        ...
        {display.length===0?(<p className="text-lg sm:text-xl md:text-2xl lg:text-3xl italic text-black mt-2">No votes yet</p>):(
